...
| "encoded packet export", "packet sink", "PacketSink", "webcodecs", "EncodedVideoChunk", "EncodedAudioChunk", "h.264 access units", "aac frames", "avcC", "AudioSpecificConfig", "rtp packetizer", "fmp4 segmenter" | [packet_sink.md](references/ez_ffmpeg/packet_sink.md) (experimental, ez-ffmpeg 0.15) |
...
|---------|---------|--------|-----------|
| ez-ffmpeg | 0.15.0 | 7.1–8.x | 1.80+ (wgpu: 1.85+) |
...
- **Automatic CLI translation (0.15)**: `cli` feature's `from_cli_args`/`emit_rust_code` cover only 6 golden-tested shapes (plain transcode/clip/audio-extract/thumbnail/`-vf scale`/single-rendition HLS) — `from_cli_args` execution is additionally gated on a verified FFmpeg runtime profile (**FFmpeg 7.1 only as of 0.15.0** — refuses to run on 8.x even for a verified shape), while `emit_rust_code` has no such gate; fall back to [cli_migration.md](references/ez_ffmpeg/cli_migration.md) for anything else
