...
| "AV1", "AVIF", "HDR", "10-bit", "modern codec", "HDR to SDR", "tone mapping", "tonemap", "washed out colors", "PQ", "HLG" | [modern_codecs.md](references/scenarios/modern_codecs.md) — HDR→SDR detection routes on `StreamInfo::Video` `color_transfer` (0.16, see [query.md](references/ez_ffmpeg/query.md)) |
...
| "progress", "progress bar", "percent complete", "ETA", "how far along", "encoding speed", "progress_handle", "ProgressHandle", "progress snapshot" | [advanced.md](references/ez_ffmpeg/advanced.md#progress-monitoring) (typed pull-based progress, ez-ffmpeg 0.16) — per-file counting for batches stays in [batch_processing.md](references/scenarios/batch_processing.md) |
...
| "run ffmpeg command in rust", "from_cli_args", "emit_rust_code", "translate ffmpeg command", "cli feature", "cli-compat", "verified shape" | [cli_compat.md](references/ez_ffmpeg/cli_compat.md) (ez-ffmpeg 0.15, `cli` feature) — for broader manual CLI-flag mapping see [cli_migration.md](references/ez_ffmpeg/cli_migration.md) |
...
**Claude**: Identify → automatic CLI translation → load `cli_compat.md` → check if the command matches one of the 6 verified shapes (plain transcode/clip/audio-extract/thumbnail/`-vf scale`/single-rendition HLS) → if yes, show `cli::from_cli_args`/`emit_rust_code`, flagging the FFmpeg-7.1-only runtime gate on execution → if no, fall back to the manual `cli_migration.md` tables
...
- **Frames in/out of memory (0.14)**: for AI/CV/ASR ingest use `frame_export::{FrameExtractor, SampleExtractor}` (packed RGB / whisper PCM, colorspace-correct); to generate video from Rust-rendered frames use `VideoWriter` — both experimental, no feature flag, no temp files (see [frame_io.md](references/ez_ffmpeg/frame_io.md)). `FrameExtractor`'s default resize/pixel-format precision changed in 0.15 to match the FFmpeg CLI (`ConversionPrecision::Standard`) — add `.conversion_precision(ConversionPrecision::High)` to reproduce 0.14.0's byte-for-byte output
- **Encoded packets straight from the encoder (0.15)**: for WebCodecs/RTP/fMP4 consumers use `packet_sink::PacketSink` — strict H.264 (libx264-only)/AAC tier, no container, blocking backpressure by design (see [packet_sink.md](references/ez_ffmpeg/packet_sink.md))
- **Automatic CLI translation (0.15)**: `cli` feature's `from_cli_args`/`emit_rust_code` cover only 6 golden-tested shapes (plain transcode/clip/audio-extract/thumbnail/`-vf scale`/single-rendition HLS) — `from_cli_args` execution is additionally gated on a verified FFmpeg runtime profile (**FFmpeg 7.1 only, still true as of 0.16.0** — refuses to run on 8.x even for a verified shape), while `emit_rust_code` has no such gate; fall back to [cli_migration.md](references/ez_ffmpeg/cli_migration.md) for anything else. The subset permanently excludes indexed per-stream options (`-c:v:0`) — port those by hand with `StreamMap`
...
- **Progress: pull typed snapshots, don't hand-roll (0.16)**: `scheduler.progress_handle()` → `snapshot()` gives per-output `frame`/`out_time`/`bitrate`/`speed` equivalents (cheap atomic reads, remux-visible, handle outlives the scheduler); metrics are `Option` — `None` means unknowable (HLS/`null` muxers report no size, packet-sink outputs report nothing), and percentage needs a caller-supplied total via `percent_of(total_us)` (see [advanced.md](references/ez_ffmpeg/advanced.md#progress-monitoring))
- **Different encoders for same-type streams in one output (0.16)**: pass `StreamMap::new(spec).codec(...).codec_opt(...)` to `add_stream_map` (FFmpeg indexed `-c:v:0`/`-b:v:0` parity); per-map settings override per-type setters key by key, and copy×re-encode conflicts fail typed at `build()` (see [video.md](references/ez_ffmpeg/video.md#per-stream-encoder-selection-streammap))
- **HDR→SDR needs tone mapping, not scaling (0.16 cookbook)**: naive `scale,format=yuv420p` yields washed-out gray; detect HDR from `StreamInfo::Video` `color_transfer` (PQ=16/HLG=18 — route on transfer, *not* primaries), probe `is_filter_available("zscale")`/`"libplacebo"`, and use the documented chains with `desat=0` + explicit `peak` (see [modern_codecs.md](references/scenarios/modern_codecs.md))
- **Graceful shutdown**: call `stop()` (blocks, flushes encoders, output is valid; 0.13 returns `Result` — check it), not `abort()` (emergency, output not guaranteed). The embedded RTMP server's `stop()` (0.16) additionally joins its threads — safe to rebind/reuse keys immediately, but never call it from inside a logger (see [streaming.md](references/ez_ffmpeg/streaming.md))
