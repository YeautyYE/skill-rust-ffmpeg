...
```toml
[dependencies]
ez-ffmpeg = { version = "0.15.0", features = ["static"] }
# OR
...
echo 'ez-ffmpeg = "0.15.0"' >> Cargo.toml
cargo build --release
```
...
cd ffmpeg_test
Add-Content Cargo.toml 'ez-ffmpeg = { version = "0.15.0", features = ["static"] }'
cargo build --release
```
...
|---------|---------|-----------------|-------------------|
| ez-ffmpeg | 0.15.0 | 7.1 – 8.x | 61.x (FFmpeg 7) / 62.x (FFmpeg 8) |
