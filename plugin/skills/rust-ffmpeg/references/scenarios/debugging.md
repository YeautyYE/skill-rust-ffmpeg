...
**Detection Keywords**: ffprobe, inspect video, metadata probe, debug ffmpeg, log levels, error messages, timebase issues, codec errors, hanging pipes, probe timeout, file integrity, corrupt file, damaged header, validate file, black frame detect, blackdetect, silence detect, silencedetect, scene detection, scene cut, cropdetect, loudness measurement, EBU R128, LUFS, true peak, content analysis, QC
...
- [Probe with Timeout](#probe-with-timeout)
- [File Integrity Validation](#file-integrity-validation)
- [Keyframe & GOP Analysis](#keyframe--gop-analysis)
- [Detection & Measurement (typed)](#detection--measurement-typed)
- [ffmpeg-next EAGAIN/EOF Handling](#ffmpeg-next-eagaineof-handling)
- [ffmpeg-sidecar Exit Status & Error Handling](#ffmpeg-sidecar-exit-status--error-handling)
- [PTS/DTS Timestamp Debugging](#ptsdts-timestamp-debugging)
...
## Detection & Measurement (typed)

Moved to its own reference: **[detection_analysis.md](detection_analysis.md)** —
the typed `analysis` API (`Analysis` one-shot runner + `MetadataEventFilter`
streaming events) for blackdetect / scene / silence / cropdetect / EBU R128,
returned as Rust structs instead of scraped `av_log` text.
