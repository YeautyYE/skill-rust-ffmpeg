...
| ez_ffmpeg/packet_sink.md | encoded packet export, packet sink, webcodecs, h.264 access units, aac frames, avcC, AudioSpecificConfig, job failure summary | PacketSink, EncodedVideoChunk, rtp packetizer, fmp4 segmenter, on_job_failed, JobFailureSummary |
...
| ez_ffmpeg/advanced.md | typed progress, progress monitoring, progress snapshot, per-output progress | ProgressHandle, progress_handle, percent_of |
| ez_ffmpeg/video.md | per-stream encoder, different codec per stream, indexed stream options | StreamMap, -c:v:0, per-map codec |
| scenarios/modern_codecs.md | HDR to SDR, tone mapping, washed out colors, PQ, HLG | tonemap, zscale, libplacebo, color_transfer |
